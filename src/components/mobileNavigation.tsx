...
const MobileNavigation = () => {
  ...
  const { isOpen, toggleOpen } = uiContext

  return (
    <div className='w-full'>
      <button onClick={toggleOpen} className='absolute top-4 right-4 z-50'>
      ...
      <div className={`relative flex justify-center items-center z-10 h-screen bg-primary transition transition-transform-all ease-in duration-500 ${isOpen ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
        <NavBar />
      </div>
    </div>
  )
